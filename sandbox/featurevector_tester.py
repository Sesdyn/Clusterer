...
if __name__ == '__main__':
    ...
    samples = import_data("LINGR_FreeFloat")
    ...
    if (inspecting == 0):
        ...
        feature = analyze(samples[inspecting_index-1])
        for k in feature:
            print k  
        x=range(samples[inspecting_index-1][1].shape[0])
        line, = plt.plot(x, samples[inspecting_index-1][1], linewidth=2)
        plt.show()

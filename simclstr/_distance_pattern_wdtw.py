...
def _distance_pattern_wdtw(list_of_ts_objects: List['TimeSeries'], metric: str = 'pattern_wdtw', distance_kwargs: dict = {}) -> Tuple[np.ndarray, List['TimeSeries']]:
    """
    ...
    metric: str, default='pattern_wdtw'
...
@njit
def _wdtw_distance(d1: np.ndarray, d2: np.ndarray, wSlopeError: float, wCurvatureError: float) -> float:
    """
    ...
    -------
    float
        Average WDTW distance per data section.
    """
    ...
    while i > 0 or j > 0:
        if i == 0:
            j -= 1
            w_path += 1
            total_weight += d2[2, j] * d1[2, i-1]
        elif j == 0:
            i -= 1
            w_path += 1
            total_weight += d1[2, i] * d2[2, j-1]
        else:
            up = dtw[i-1, j]
            left = dtw[i, j-1]
            diag = dtw[i-1, j-1]
